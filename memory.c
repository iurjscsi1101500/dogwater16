...
inline uint16_t write(struct CPU *cpu, const uint16_t mem ,const uint16_t r_src, const uint16_t offset) {
...
inline uint16_t push(struct CPU *cpu, const uint16_t r_src) {
    cpu->mem[--cpu->sp] = cpu->regs[r_src];
    return cpu->mem[cpu->sp];
}
inline uint16_t pop(struct CPU *cpu, const uint16_t r_dest) {
    cpu->regs[r_dest] = cpu->mem[cpu->sp++];
    return cpu->regs[r_dest];
}
